...
const apiBaseStorageKey = "minta-duit-api-base";
let activeFrame = "h1";
...
let apiBaseUrl = restoreApiBaseUrl();
...
function snapshotUrl() {
  return apiBaseUrl ? `${apiBaseUrl}/api/snapshot` : "./api/snapshot";
}
...
async function refreshFromBridge() {
  ...
  try {
    ...
    const needsServer = window.location.hostname.endsWith("github.io") && !apiBaseUrl;
    statusLine.textContent = needsServer ? "Server belum diset: buka link dengan ?api=https://server-vps" : "Auto belum connect ke MT4 bridge";
  }
}
